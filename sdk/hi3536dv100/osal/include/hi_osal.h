...
extern int osal_atomic_dec_and_test(osal_atomic_t *atomic);
...
extern int osal_wait_timeout_uninterruptible(osal_wait_t * wait, osal_wait_cond_func_t func, void* param, unsigned long ms);

extern signed long osal_schedule_timeout(signed long timeout);
...
//osal_wait_event_uninterruptible_timeout-->wait_event_uninterruptible_timeout
#define osal_wait_event_uninterruptible_timeout(wait, func, param, timeout)             \
({                                  \
    int __ret = timeout;                          \
                                        \
    if ((func(param)) && !timeout) \
...
extern void osal_disable_irq(unsigned int irq);
extern void osal_enable_irq(unsigned int irq);
...
extern int osal_kthread_should_stop(void);

//tasklet api
typedef struct osal_tasklet{
    void *tasklet_struct;
}osal_tasklet_t;
extern int osal_tasklet_init(osal_tasklet_t *tasklet, void (*func)(unsigned long), unsigned long data);
extern void osal_tasklet_kill(osal_tasklet_t *tasklet);
extern void osal_tasklet_schedule(osal_tasklet_t *tasklet);
extern int osal_get_current_tgid(void);
extern int osal_get_current_pid(void);
...
//addr translate
extern void *osal_ioremap(unsigned long phys_addr, int size);
extern void *osal_ioremap_nocache(unsigned long phys_addr, int size);
extern void *osal_ioremap_cached(unsigned long phys_addr, int size);
...
//math
extern unsigned int osal_do_div(unsigned int n, unsigned int base);
extern unsigned long long osal_do_div64(unsigned long long n, unsigned int base);
extern unsigned long long osal_do_div_rem64(unsigned long long n, unsigned int base);

extern long long osal_do_div64_s64(long long dividend, long long divisor);
extern unsigned long long osal_do_div64_u64(unsigned long long dividend, unsigned long long divisor);
extern unsigned int osal_iter_div_u64_rem(unsigned long long  dividend, unsigned int divisor, unsigned long long  *remainder);
...
extern int osal_is_enable_hrtimer(void);
...
extern int osal_pfn_valid(unsigned int pfn);
