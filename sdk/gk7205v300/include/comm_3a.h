...
#define IS_HDR_MODE(mode) \
	(((mode) > DYNAMIC_RANGE_SDR10) && ((mode) < DYNAMIC_RANGE_BUTT))
#define IS_BUILT_IN_WDR_MODE(mode) ((mode) == WDR_MODE_BUILT_IN)
#define IS_FS_WDR_MODE(mode)                                         \
	((!IS_LINEAR_MODE(mode)) && (!IS_BUILT_IN_WDR_MODE(mode)) && \
	 ((mode) < WDR_MODE_BUTT))
#define IS_2to1_WDR_MODE(mode)                        \
	(((mode) == WDR_MODE_2To1_FRAME) ||           \
	 ((mode) == WDR_MODE_2To1_FRAME_FULL_RATE) || \
	 ((mode) == WDR_MODE_2To1_LINE) || ((mode) == WDR_MODE_QUDRA))
#define IS_3to1_WDR_MODE(mode)                        \
	(((mode) == WDR_MODE_3To1_FRAME) ||           \
...
#define IS_FULL_WDR_MODE(mode)                        \
	(((mode) == WDR_MODE_2To1_FRAME_FULL_RATE) || \
	 ((mode) == WDR_MODE_3To1_FRAME_FULL_RATE) || \
	 ((mode) == WDR_MODE_4To1_FRAME_FULL_RATE))
#define IS_HALF_WDR_MODE(mode)                                                 \
	(((mode) == WDR_MODE_2To1_FRAME) || ((mode) == WDR_MODE_3To1_FRAME) || \
	 ((mode) == WDR_MODE_4To1_FRAME))
#define IS_LINE_WDR_MODE(mode)                                               \
	(((mode) == WDR_MODE_2To1_LINE) || ((mode) == WDR_MODE_3To1_LINE) || \
...
typedef struct ISP_FE_AE_STITCH_STAT_3_S {
	GK_U16 au16ZoneAvg[VI_MAX_PIPE_NUM][ISP_CHN_MAX_NUM][AE_ZONE_ROW]
			  [AE_ZONE_COLUMN][4];
...
typedef struct ISP_AE_EXP_FUNC_S {
	GK_S32 (*pfn_ae_init)
	(GK_S32 s32Handle, const ISP_AE_PARAM_S *pstAeParam);
	GK_S32 (*pfn_ae_run)
	(GK_S32 s32Handle, const ISP_AE_INFO_S *pstAeInfo,
...
typedef struct ISP_AWB_EXP_FUNC_S {
	GK_S32 (*pfn_awb_init)
	(GK_S32 s32Handle, const ISP_AWB_PARAM_S *pstAwbParam,
	 ISP_AWB_RESULT_S *pstAwbResult);
	GK_S32 (*pfn_awb_run)
	(GK_S32 s32Handle, const ISP_AWB_INFO_S *pstAwbInfo,
	 ISP_AWB_RESULT_S *pstAwbResult, GK_S32 s32Rsv);
	GK_S32 (*pfn_awb_ctrl)
	(GK_S32 s32Handle, GK_U32 u32Cmd, GK_VOID *pValue);
