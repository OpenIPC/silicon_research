...
GK_S32 GK_API_SYS_UnBind(const MPP_CHN_S *pstSrcChn,
			 const MPP_CHN_S *pstDestChn);
GK_S32 GK_API_SYS_GetBindbyDest(const MPP_CHN_S *pstDestChn,
				MPP_CHN_S *pstSrcChn);
GK_S32 GK_API_SYS_GetBindbySrc(const MPP_CHN_S *pstSrcChn,
...
GK_S32 GK_API_SYS_MmzAlloc(GK_U64 *pu64PhyAddr, GK_VOID **ppVirAddr,
			   const GK_CHAR *strMmb, const GK_CHAR *strZone,
			   GK_U32 u32Len);

GK_S32 GK_API_SYS_MmzAlloc_Cached(GK_U64 *pu64PhyAddr, GK_VOID **ppVirAddr,
				  const GK_CHAR *pstrMmb,
				  const GK_CHAR *pstrZone, GK_U32 u32Len);

GK_S32 GK_API_SYS_MmzFree(GK_U64 u64PhyAddr, GK_VOID *pVirAddr);

GK_S32 GK_API_SYS_MmzFlushCache(GK_U64 u64PhyAddr, GK_VOID *pVirAddr,
				GK_U32 u32Size);
...
GK_S32 GK_API_SYS_MflushCache(GK_U64 u64PhyAddr, GK_VOID *pVirAddr,
			      GK_U32 u32Size);

GK_S32 GK_API_SYS_SetMemConfig(const MPP_CHN_S *pstMppChn,
...
GK_S32 GK_API_SYS_GetVirMemInfo(const void *pVirAddr,
				SYS_VIRMEM_INFO_S *pstMemInfo);

GK_S32
GK_API_SYS_SetScaleCoefLevel(const SCALE_RANGE_S *pstScaleRange,
			     const SCALE_COEFF_LEVEL_S *pstScaleCoeffLevel);
GK_S32 GK_API_SYS_GetScaleCoefLevel(const SCALE_RANGE_S *pstScaleRange,
...
GK_S32 GK_API_SYS_GetVPSSVENCWrapBufferLine(VPSS_VENC_WRAP_PARAM_S *pWrapParam,
					    GK_U32 *pu32BufLine);

GK_S32 GK_API_LOG_SetLevelConf(LOG_LEVEL_CONF_S *pstConf);
GK_S32 GK_API_LOG_GetLevelConf(LOG_LEVEL_CONF_S *pstConf);

GK_S32 GK_API_SYS_SetRawFrameCompressParam(
	const RAW_FRAME_COMPRESS_PARAM_S *pstCompressParam);
GK_S32 GK_API_SYS_GetRawFrameCompressParam(
	RAW_FRAME_COMPRESS_PARAM_S *pstCompressParam);
