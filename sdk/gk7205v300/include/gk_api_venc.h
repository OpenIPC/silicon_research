...
GK_S32 GK_API_VENC_StartRecvFrame(VENC_CHN VeChn,
...
GK_S32 GK_API_VENC_SetChnAttr(VENC_CHN VeChn,
			      const VENC_CHN_ATTR_S *pstChnAttr);
GK_S32 GK_API_VENC_GetChnAttr(VENC_CHN VeChn, VENC_CHN_ATTR_S *pstChnAttr);

GK_S32 GK_API_VENC_GetStream(VENC_CHN VeChn, VENC_STREAM_S *pstStream,
			     GK_S32 s32MilliSec);

GK_S32 GK_API_VENC_ReleaseStream(VENC_CHN VeChn, VENC_STREAM_S *pstStream);

GK_S32 GK_API_VENC_InsertUserData(VENC_CHN VeChn, GK_U8 *pu8Data,
				  GK_U32 u32Len);

GK_S32 GK_API_VENC_SendFrame(VENC_CHN VeChn, const VIDEO_FRAME_INFO_S *pstFrame,
			     GK_S32 s32MilliSec);
GK_S32 GK_API_VENC_SendFrameEx(VENC_CHN VeChn,
...
GK_S32 GK_API_VENC_GetSSERegion(VENC_CHN VeChn, GK_U32 u32Index,
				VENC_SSE_CFG_S *pstSSECfg);
GK_S32 GK_API_VENC_SetSSERegion(VENC_CHN VeChn,
				const VENC_SSE_CFG_S *pstSSECfg);

GK_S32 GK_API_VENC_SetChnParam(VENC_CHN VeChn,
...
GK_S32 GK_API_VENC_GetForegroundProtect(
	VENC_CHN VeChn, VENC_FOREGROUND_PROTECT_S *pstForegroundProtect);
GK_S32 GK_API_VENC_SetForegroundProtect(
	VENC_CHN VeChn, const VENC_FOREGROUND_PROTECT_S *pstForegroundProtect);

GK_S32 GK_API_VENC_SetSceneMode(VENC_CHN VeChn,
				const VENC_SCENE_MODE_E enSceneMode);
GK_S32 GK_API_VENC_GetSceneMode(VENC_CHN VeChn,
...
GK_S32 GK_API_VENC_SetCuPrediction(VENC_CHN VeChn,
				   const VENC_CU_PREDICTION_S *pstCuPrediction);
GK_S32 GK_API_VENC_GetCuPrediction(VENC_CHN VeChn,
				   VENC_CU_PREDICTION_S *pstCuPrediction);

GK_S32 GK_API_VENC_SetSkipBias(VENC_CHN VeChn,
			       const VENC_SKIP_BIAS_S *pstSkipBias);
GK_S32 GK_API_VENC_GetSkipBias(VENC_CHN VeChn, VENC_SKIP_BIAS_S *pstSkipBias);

GK_S32
GK_API_VENC_SetDeBreathEffect(VENC_CHN VeChn,
			      const VENC_DEBREATHEFFECT_S *pstDeBreathEffect);
GK_S32 GK_API_VENC_GetDeBreathEffect(VENC_CHN VeChn,
				     VENC_DEBREATHEFFECT_S *pstDeBreathEffect);

GK_S32
GK_API_VENC_SetHierarchicalQp(VENC_CHN VeChn,
			      const VENC_HIERARCHICAL_QP_S *pstHierarchicalQp);
GK_S32 GK_API_VENC_GetHierarchicalQp(VENC_CHN VeChn,
				     VENC_HIERARCHICAL_QP_S *pstHierarchicalQp);

GK_S32 GK_API_VENC_SetRcAdvParam(VENC_CHN VeChn,
				 const VENC_RC_ADVPARAM_S *pstRcAdvParam);
GK_S32 GK_API_VENC_GetRcAdvParam(VENC_CHN VeChn,
				 VENC_RC_ADVPARAM_S *pstRcAdvParam);

GK_S32 GK_API_VENC_EnableSvc(VENC_CHN VeChn, GK_BOOL bEnable);
GK_S32 GK_API_VENC_SetSvcParam(VENC_CHN VeChn,
			       const VENC_SVC_PARAM_S *pstSvcParam);
GK_S32 GK_API_VENC_GetSvcParam(VENC_CHN VeChn, VENC_SVC_PARAM_S *pstSvcParam);
GK_S32
GK_API_VENC_GetSvcSceneComplexity(VENC_CHN VeChn,
				  VENC_SVC_DETECT_RESULT_S *pstSvcDetectResult);
GK_S32 GK_API_VENC_SendSvcRegion(VENC_CHN VeChn,
				 VENC_SVC_RECT_INFO_S *pstSvcRegion);

GK_S32 GK_API_VENC_SetSearchWindow(VENC_CHN VeChn,
				   const VENC_SEARCH_WINDOW_S *pstSearchWindow);
GK_S32 GK_API_VENC_GetSearchWindow(VENC_CHN VeChn,
				   VENC_SEARCH_WINDOW_S *pstSearchWindow);
