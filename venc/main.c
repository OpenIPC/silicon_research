...
void printHelp() {
	printf(
		...
		"        200_imx307F  - v200, IMX307, 2-lane MIPI | 1080p | 30  fps "
		"only\n"
		"\n"
		"      GK7205v300 / IMX307\n"
		"        300_imx307B  - v300, IMX307, 4-lane MIPI | 720p  | any fps\n"
		"        300_imx307F  - v300, IMX307, 4-lane MIPI | 1080p | 30  fps "
		"only\n"
		"\n"
		"      GK7205v300 / IMX335\n"
		"        300_imx335F  - v300, IMX335, 4-lane MIPI | 4MP   | 30  fps "
		"only\n"
		"\n"
...
int main(int argc, const char* argv[]) {
	...
	// --- Board configuration
	// --------------------------------------------------------------
	/*combo_dev_attr_t* mipi_profile        =
	&MIPI_4lane_CHN0_SENSOR_IMX335_10BIT_4M_WDR2TO1_ATTR; ISP_PUB_ATTR_S*
	isp_profile         = &ISP_PROFILE_IMX335_MIPI_4M_30FPS; ISP_SNS_OBJ_S*
	sns_object          = &stSnsImx335Obj; VI_DEV_ATTR_S*    sns_profile =
	&DEV_ATTR_IMX335_4M_BASE;
	...
	uint8_t *mem = mmap64(NULL, page_offset + len, PROT_READ | PROT_WRITE,
   MAP_PRIVATE, fd, page_base); if (mem == MAP_FAILED) { perror("Can't map
   memory"); return -1;
	}
	...
	*/
	...
	printf("> Starting\n"
		   ...
		   "  - Sensor  : %d x %d @ %d\n"
		   "  - Image   : %d x %d\n",
		goke_version, sensor_width, sensor_height, sensor_framerate,
	...
	vb_conf.astCommPool[0].u64BlkSize =
		VI_GetRawBufferSize(sensor_width, sensor_height,
	...
	// - NOT FOR: BT1120_2M_30FPS_8BIT / BT656_2M_30FPS_8BIT /
	// BT601_2M_30FPS_8BIT
	{
...

	{
		...
		int ret = HI_MPI_VPSS_GetLowDelayAttr(
			vpss_group_id, vpss_second_ch_id, &info);
		if (enable_lowdelay) {
			info.bEnable = HI_TRUE;
			info.u32LineCnt = image_height / 4;
			ret = HI_MPI_VPSS_SetLowDelayAttr(
				vpss_group_id, vpss_second_ch_id, &info);
			if (ret != HI_SUCCESS) {
		...
		ret = HI_MPI_VPSS_GetLowDelayAttr(
			vpss_group_id, vpss_second_ch_id, &info);
	...
	// - Configure h264 encoder
	{
		...
		config.stVencAttr.u32MaxPicWidth =
			image_width; // - Max image size for dynamic resolution selection
		...
		config.stVencAttr.u32BufSize = ALIGN_UP(
			image_width * image_height * 3 / 4, 64); /*stream buffer size*/
		config.stVencAttr.u32Profile = 0; // - Baseline (0), Main(1), High(1)
		config.stVencAttr.bByFrame =
			venc_by_frame; // - TRUE for get per-frame, FALSE for get per-slice
	...
	// - Configure rate control for channel #1
	{
		...
		HI_MPI_VENC_GetRcParam(venc_second_ch_id, &param);
		printf("> Scene detect = %s, Adaptive IDR = %s, Start Qp = %d, Row dQp "
			   "= %d\n",
	...
	case PT_H264: {
		...
		if (enable_slices) {
			if (venc_by_frame) {
				printf(
					"WARN: Slices are not available in [frame] data format\n");
			} else {
				int ret =
					HI_MPI_VENC_SetH264SliceSplit(venc_second_ch_id, &param);
				if (ret != HI_SUCCESS) {
	...
	case PT_H265: {
		...
		if (enable_slices) {
			if (venc_by_frame) {
				...
				int ret =
					HI_MPI_VENC_SetH265SliceSplit(venc_second_ch_id, &param);
				if (ret != HI_SUCCESS) {
	...
	/*{ VENC_INTRA_REFRESH_S intra;
	  ...
	  printf("> Intra refresh EN = %d, Num = %d, Mode = %d, IQp = %d\n",
		intra.bRefreshEnable, intra.u32RefreshNum, intra.enIntraRefreshMode,
	intra.u32ReqIQp
	  );
...
int processStream(VENC_CHN channel_id, int socket_handle,
	...
	// - Per-Frame mode may return multiple slices, we need to allocate memory
	// for all reported slices.
	...
	for (uint32_t i = 0; i < stream.u32PackCount; i++) {
		// printf("> PTS: %ld\n", current_pts);
		// printf("> SEQ: %d : %d, Size = %d, NAL = ", stream.u32Seq,  i,
		// stream.pstPack[i].u32Len  - stream.pstPack[i].u32Offset);

		// printf("> NUM = %d / %d\n", stream.pstPack[0].u32DataNum,
		// stream.u32PackCount);
...
void transmit(int socket_handle, uint8_t* tx_buffer, uint32_t tx_size,
	...
	// - Compact mode
	case 0: {
		sendto(socket_handle, tx_buffer, tx_size, 0, dst_address,
...
void sendPacket(uint8_t* pack_data, uint32_t pack_size, int socket_handle,
	...
	if (pack_data[0] == 0 && pack_data[1] == 0 && pack_data[2] == 0 &&
		pack_data[3] == 1) {
		uint8_t prefix = 4;
		pack_data += prefix;
		pack_size -= prefix;
		...
		if (pack_size > max_size + prefix) {
			...
			while (pack_size) {
				uint32_t chunk_size =
					pack_size > max_size ? max_size : pack_size;
				...
				transmit(socket_handle, tx_buffer, chunk_size + tx_size,
					dst_address);
