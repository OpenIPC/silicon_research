...
struct _fbg* fbg_fbdevSetup(char* fb_device, int page_flipping,
	...
	struct _fbg_fbdev_context* fbdev_context =
		(struct _fbg_fbdev_context*)calloc(
			1, sizeof(struct _fbg_fbdev_context));
	if (!fbdev_context) {
	...
	if (ioctl(fbdev_context->fd, FBIOGET_VSCREENINFO, &fbdev_context->vinfo) ==
		-1) {
		fprintf(stderr,
			"fbg_fbdevSetup: '%s' Cannot obtain framebuffer "
	...
	if (ioctl(fbdev_context->fd, FBIOPUT_VSCREENINFO, &fbdev_context->vinfo) ==
		-1) {
		fprintf(stderr,
			"fbg_fbdevSetup: '%s' Unable to set VSCREENINFO informations!\n",
			fb_device);
		close(fbdev_context->fd);
		return NULL;
	}
	...
	if (ioctl(fbdev_context->fd, FBIOGET_FSCREENINFO, &fbdev_context->finfo) ==
		-1) {
		fprintf(stderr,
			"fbg_fbdevSetup: '%s' Cannot obtain framebuffer "
	...
	// --- OK
	fprintf(stdout,
		"fbg_fbdevSetup: '%s' (%dx%d (%dx%d virtual) %d bpp (%d/%d, %d/%d, "
	...
	if (fbdev_context->vinfo.bits_per_pixel != 16 &&
		...
		fbdev_context->vinfo.bits_per_pixel != 32) {
		fprintf(stderr,
			"fbg_fbdevSetup: '%s' Unsupported format (only 16, 24 or 32 bits "
	...
	if (fbdev_context->vinfo.bits_per_pixel == 16) {
		fprintf(stdout,
			"fbg_fbdevSetup: 16 bpp framebuffer detected; page flipping option "
	...
	if (page_flipping) {
		...
		if (ioctl(fbdev_context->fd, FBIOPAN_DISPLAY, &fbdev_context->vinfo) ==
			-1) {
			fprintf(stderr,
				"fbg_fbdevSetup: '%s' FBIOPAN_DISPLAY / page flipping not "
				"supported!\n",
				fb_device);
		} else {
			// double the virtual height
			fbdev_context->vinfo.yres_virtual =
				fbdev_context->vinfo.yres_virtual * 2;
			if (ioctl(fbdev_context->fd, FBIOPUT_VSCREENINFO,
					&fbdev_context->vinfo) == -1) {
				fprintf(stderr,
					"fbg_fbdevSetup: '%s' FBIOPUT_VSCREENINFO failed, page "
					"flipping disabled!\n",
					fb_device);
			} else {
				...
				fprintf(stdout,
					"fbg_fbdevSetup: '%s' Page flipping enabled (virtual "
					"height was doubled)!\n",
					fb_device);

				if (ioctl(fbdev_context->fd, FBIOGET_FSCREENINFO,
						&fbdev_context->finfo) == -1) {
					fprintf(stderr,
						"fbg_fbdevSetup: '%s' Cannot obtain framebuffer "
						"FBIOGET_FSCREENINFO informations!\n",
						fb_device);
	...
	fbdev_context->buffer =
		(unsigned char*)mmap(0, fbdev_context->finfo.smem_len,
	...
	if (fbdev_context->page_flipping) {
		...
		fbg->back_buffer =
			fbdev_context->buffer + fbg->width * fbg->components * fbg->height;
	} else {
