			      ...
			      int bitness)
{
  ...
  //printf("MIPI LANES: %d, FPS: %d\n", mipi_lanes, fps);
	...
	// HMAX
	if (winmode == WINMODE_720P) {
		if (fps == 25)			imx307_write_adjacent(ViPipe, 0x301C, 0x1EF0);
		else if (fps == 30) imx307_write_adjacent(ViPipe, 0x301C, 0x19C8);
		else if (fps == 50) imx307_write_adjacent(ViPipe, 0x301C, 0x0F78);
		else if (fps == 60) imx307_write_adjacent(ViPipe, 0x301C, 0x0CE4);
